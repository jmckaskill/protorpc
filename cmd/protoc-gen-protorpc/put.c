...
void do_maxsz(str_t *o, const struct type *t, bool define) {
    str_addf(o, "int pb_maxsz_%s(%s const *m)", t->proto_suffix.buf, t->c_type.buf);
    if (!define) {
        str_add(o, ";" EOL);
        return;
    }
    ...
    for (int i = 0; i < t->msg->field.len; i++) {
        ...
        if (f->oneof_index_set) {
			...
			str_addf(o, "\tif (pb_nonzero_%s(&%s)) {" EOL, ft->proto_suffix.buf, mbr.buf);
        } else {
        ...
        if (f->label == LABEL_REPEATED) {
            if (is_field_packed(t, f)) {
                str_addf(o, "\t\tn += %u + (%d * %s.len);" EOL, tagsz, (int) ft->max_proto_size, mbr.buf);
            } else if (isfinite(ft->max_proto_size)) {
                str_addf(o, "\t\tn += (%u + %d) * %s.len;" EOL, tagsz, (int) ft->max_proto_size, mbr.buf);
            } else if (ft->msg) {
                str_addf(o, "\t\tfor (int i = 0; i < %s.len; i++) {" EOL, mbr.buf);
                str_addf(o, "\t\t\tn += %u + pb_maxsz_%s(%s.v[i]);" EOL, tagsz, ft->proto_suffix.buf, mbr.buf);
                str_add(o, "\t\t}" EOL);
            } else {
				// bytes and strings
				str_addf(o, "\t\tfor (int i = 0; i < %s.len; i++) {" EOL, mbr.buf);
				str_addf(o, "\t\t\tn += %u + 5 + %s.v[i].len;" EOL, tagsz, mbr.buf);
				str_add(o, "\t\t}" EOL);
			}
        } else if (isfinite(ft->max_proto_size)) {
            str_addf(o, "\t\tn += %u + %d;" EOL, tagsz, (int) ft->max_proto_size);
        } else if (ft->msg) {
            str_addf(o, "\t\tn += %u + pb_maxsz_%s(%s);" EOL, tagsz, ft->proto_suffix.buf, mbr.buf);
        } else {
            // bytes and strings
            str_addf(o, "\t\tn += %u + 5 + %s.len;" EOL, tagsz, mbr.buf);
        }
...
void do_encode(str_t *o, const struct type *t, bool define) {
    str_addf(o, "char *pb_encode_%s(char *p, %s const *m)", t->proto_suffix.buf, t->c_type.buf);
    if (!define) {
        str_add(o, ";" EOL);
        return;
    }
    ...
    for (int i = 0; i < t->msg->field.len; i++) {
		...
		if (f->label == LABEL_REPEATED) {
			if (ft->pod_message) {
				...
				str_addf(o, "\t\t\tp = pb_encode_%s(start, &%s.v[i]);" EOL, ft->proto_suffix.buf, mbr.buf);
				str_add(o, "\t\t\tstart[-1] = (char) (p - start);" EOL);
				str_add(o, "\t\t}" EOL);
			} else if (ft->msg) {
				str_addf(o, "\t\tfor (int i = 0; i < %s.len; i++) {" EOL, mbr.buf);
				str_addf(o, "\t\t\tp = pb_put_tag_%u(p, %u);" EOL, tagsz, tag);
				str_addf(o, "\t\t\tint szlen = pb_u32_size(%s.v[i]->pb_hdr.maxsz);" EOL, mbr.buf);
				...
				str_addf(o, "\t\tp = pb_put_tag_%u(p, %u);" EOL, tagsz, tag);
				str_addf(o, "\t\tp = pb_put_packed_%s(p, ", ft->proto_suffix.buf);
				get_proto_cast(o, f, 1, 1);
				str_addf(o, " %s.v, %s.len);" EOL, mbr.buf, mbr.buf);
			} else {
				str_addf(o, "\t\tfor (int i = 0; i < %s.len; i++) {" EOL, mbr.buf);
				str_addf(o, "\t\t\tp = pb_put_tag_%u(p, %u);" EOL, tagsz, tag);
				str_addf(o, "\t\t\tp = pb_put_%s(p, ", ft->proto_suffix.buf);
			...
			str_addf(o, "\t\tp = pb_put_tag_%u(p, %u);" EOL, tagsz, tag);
			str_add(o, "\t\tchar *start = p+1;" EOL);
			str_addf(o, "\t\tp = pb_encode_%s(start, &%s);" EOL, ft->proto_suffix.buf, mbr.buf);
			str_add(o, "\t\tstart[-1] = (char) (p - start);" EOL);
        } else if (ft->msg) {
            str_addf(o, "\t\tp = pb_put_tag_%u(p, %u);" EOL, tagsz, tag);
            str_addf(o, "\t\tint szlen = pb_u32_size(%s->pb_hdr.maxsz);" EOL, mbr.buf);
            str_add(o, "\t\tchar *start = p+szlen;" EOL);
			str_addf(o, "\t\tp = pb_encode_%s(start, %s);" EOL, ft->proto_suffix.buf, mbr.buf);
            str_add(o, "\t\tpb_put_finish(p, start, szlen);" EOL);
        } else {
