...
void do_server(str_t *o, const struct type *t, bool define) {
	if (!define) {
		...
		for (int i = 0; i < t->svc->method.len; i++) {
			...
			if (!is_empty(out)) {
				str_addf(o, ", %s *out", out->c_type.c_str);
			}
			str_addf(o, ");" EOL);
		}
		str_add(o, "};" EOL);
	...
	case 1:
		...
		for (int i = 0; i < t->svc->method.len; i++) {
			...
			if (!m->client_streaming && !m->server_streaming) {
				str_addf(o, "static inline int pb_rpc_%s(pb_alloc_t *obj, str_t *resp, char *body, int bodysz) {" EOL, name.c_str);
				if (is_empty(in)) {
					str_add(o, "\t(void) body; (void) bodysz;" EOL);
				} else {
					str_addf(o, "\t%s in = {0};" EOL, in->c_type.c_str);
					str_add(o, "\t(void) bodysz;" EOL);
					if (in->pod_message) {
						str_add(o, "\t(void) obj;" EOL);
						str_addf(o, "\tif (pb_parse_%s(body, &in) == pb_errret) {" EOL, in->json_suffix.c_str);
					} else {
						str_addf(o, "\tif (pb_parse_%s(body, obj, &in) == pb_errret) {" EOL, in->json_suffix.c_str);
					}
				...
				if (!is_empty(out)) {
					str_addf(o, "\t%s out = {0};" EOL, out->c_type.c_str);
				}
				str_addf(o, "\tint sts = rpc_%s(obj", name.c_str);
				if (!is_empty(out)) {
					str_add(o, ", &out");
				}
				if (!is_empty(in)) {
				...
				if (is_empty(out)) {
					...
					str_addf(o, "\tpb_print_%s(resp, &out);" EOL, out->json_suffix.c_str);
				}
				str_add(o, "\treturn sts;" EOL);
				str_add(o, "}" EOL);
			}
		}
		str_addf(o, "static inline void register_%s(struct rpc_server *s) {" EOL, t->c_type.c_str);
		for (int i = 0; i < t->svc->method.len; i++) {
			const struct MethodDescriptorProto *m = t->svc->method.v[i];
			...
			if (m->server_streaming) {
				str_addf(o, "\tg_rpc_%s = rpc_publisher_new();" EOL, name.c_str);
				str_addf(o, "\trpc_handle_stream(s, \"/api/%s/%s\", g_rpc_%s);" EOL, t->proto_type.c_str+1, m->name.c_str, name.c_str);
			} else {
				str_addf(o, "\trpc_handle_post(s, \"/api/%s/%s\", &pb_rpc_%s);" EOL, t->proto_type.c_str+1, m->name.c_str, name.c_str);
			}
		}
		str_add(o, "}" EOL);
		break;
	case 2:
		...
		for (int i = 0; i < t->svc->method.len; i++) {
			...
			if (m->server_streaming) {
				str_addf(o, "struct rpc_publisher *g_rpc_%s;" EOL, name.c_str);
				str_addf(o, "void publish_%s(%s const *msg) {" EOL, name.c_str, out->c_type.c_str);
				str_addf(o, "\tstr_t *s = rpc_publish_start(g_rpc_%s);" EOL, name.c_str);
				str_add(o, "\tif (msg) {" EOL);
				str_addf(o, "\t\tpb_print_%s(s, msg);" EOL, out->json_suffix.c_str);
				str_add(o, "\t}" EOL);
				str_addf(o, "\trpc_publish_finish(g_rpc_%s);" EOL, name.c_str);
				str_add(o, "}" EOL);
			} else {
				str_addf(o, "int cgi_%s(int sts", name.c_str);
				if (!is_empty(out)) {
					str_addf(o, ", %s const *msg", out->c_type.c_str);
				}
				str_add(o, ") {" EOL);
				...
				if (!is_empty(out)) {
					str_add(o, "\tif (msg) {" EOL);
					str_addf(o, "\t\tpb_print_%s(&out, msg);" EOL, out->json_suffix.c_str);
					str_add(o, "\t}" EOL);
				}
				str_add(o, "\tprintf(\"HTTP/1.1 %d \\r\\nContent-Length:%u\\r\\n\\r\\n\", sts, out.len);" EOL);
				str_add(o, "\tfwrite(out.c_str, 1, out.len, stdout);" EOL);
